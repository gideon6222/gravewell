...
## Drown, at the two moments that matter: the surface, and well under it.
##
##   godot --path . --resolution 460x996 --script res://scripts/shot_drown.gd -- 0
##
## 0 is the ship dropping through the waterline, which is the frame that has to
## say "water" without a caption. 1 is twenty metres down, where the lamp has
## closed in and the hull is going.
...
func _initialize() -> void:
	...
	_main.sim = Sim.new(3, Classes.DROWN)
	_main._rebind()
	var w: World = _main.sim.world
	var table := int(w.water_depth())
...
func _process(_delta: float) -> bool:
	...
	if _frames < 6:
		return false
	root.get_texture().get_image().save_png("user://drown%d.png" % _stage)
	...
	print("wrote drown%d.png  depth %.1f  water %.1f  submerged %s" % [
		_stage, _main.sim.flight.depth(), _main.sim.world.water_depth(),
